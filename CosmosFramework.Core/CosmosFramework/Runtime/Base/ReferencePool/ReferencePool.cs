...
namespace Cosmos
{
    public static class ReferencePool
    {
            ...
            where T : class
        {
            ...
            readonly ConcurrentPool<T> pool;
            ...
            {
                referenceType = type;
                pool = new ConcurrentPool<T>(objectGenerator, onRelease);
            }
            public T Accquire()
            {
                poolAccquireCount++;
        ...
        static readonly  ConcurrentDictionary<Type, ReferPool<IReference>> referencePoolDict
            = new ConcurrentDictionary<Type, ReferPool<IReference>>();
        /// <summary>
        /// 清除所有池；
        /// </summary>
        ...
        static ReferPool<IReference> GetReferencePool(Type type)
        {
            ...
            if (!referencePoolDict.TryGetValue(type, out var pool))
            {
                ...
                referencePoolDict.TryAdd(type, pool);
            }
            return pool;
        }
    }
